...
export const LimelightNav = ({
  ...
  return (
    <nav className={cn('relative inline-flex items-center h-16 rounded-full bg-background/50 backdrop-blur-md border border-white/10 text-foreground px-4 shadow-lg', className)}>
      {items.map(({ id, icon, label, onClick }, index) => (
        <a
          key={id}
          ref={el => { navItemRefs.current[index] = el; }}
          className={cn('relative z-20 flex h-full cursor-pointer items-center justify-center p-5', iconContainerClassName)}
          onClick={() => handleItemClick(index, onClick)}
          aria-label={label}
        >
          {cloneElement(icon as React.ReactElement<any>, {
            className: cn(
              'w-7 h-7 transition-opacity duration-100 ease-in-out',
